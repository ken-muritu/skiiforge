...
| **An SDL2 app's embedded video-preview widget renders blank under native Wayland, even though the process is genuinely healthy** | `guvcview` (webcam viewer/recorder) opened a real window, held a real read-write fd on `/dev/video0` (confirmed via `lsof`), and logged no fatal error — yet the preview area stayed black, user-confirmed. `XDG_SESSION_TYPE=wayland` was the tell: SDL2's window-embedding trick for the preview widget depends on X11-specific parenting that has no native-Wayland equivalent, so the camera pipeline runs correctly while only the rendered frame never reaches the screen. | A live process + an open device fd is proof the *capture* path works, not proof the *display* path works — treat "window open, feed black" as a renderer problem, not a camera/permissions one, and check `$XDG_SESSION_TYPE` before debugging further upstream. Fix: force the SDL app through XWayland's more complete X11 compatibility layer with `SDL_VIDEODRIVER=x11` (confirmed fixed live). Bake it into a **user-level** `~/.local/share/applications/<name>.desktop` override (same filename as the system one in `/usr/share/applications/`, which takes precedence for that user) with `Exec=env SDL_VIDEODRIVER=x11 guvcview` — editing the system `.desktop` directly gets clobbered on the next package upgrade. |
